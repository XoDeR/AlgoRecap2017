...
// Function to find maximum value of the expression (A[l] - A[k] + A[j] - A[i])
// where l > k > j > i
int maximizeExpression(int A[], int n)
{
	...
	L1[n - 2] = L2[n - 2] = L3[n - 2] = L1[n - 1] = L2[n - 1] = L1[n] = INT_MIN;
	...
	// Uncomment below code to print lookup tables
	/*
		printArray(L1, n + 1);
		printArray(L2, n);
		printArray(L3, n - 1);
