...
int longestPalindrome(string X, int i, int j)
{
	...
	if (X[i] == X[j])
		...
		// and recuse for the remaining substring X[i+1, j-1]
		return longestPalindrome(X, i + 1, j - 1) + 2;

	// if last character of string is different to the first character

	// 1. Remove last character & recurse for the remaining substring X[i, j-1]
	// 2. Remove first character & recurse for the remaining substring X[i+1, j]
